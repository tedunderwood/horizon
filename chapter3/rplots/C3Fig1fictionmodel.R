library(scales)
library(ggplot2)
l <- read.csv('~/Dropbox/book/chapter3/results/fullfiction.results.csv')
...
p <- ggplot(l, aes(x = pubdate, y = logistic, color = reviewed, shape = reviewed)) + theme_bw() +
  ...
  ggtitle('Predicted probability of belonging to reviewed set') +
  theme(plot.title = element_text(hjust = 0, margin = margin(b = 10), size = 16))

tiff("~/Dropbox/book/chapter3/images/fictionmodel.tiff", height = 6, width = 9, units = 'in', res=400)
plot(p)
dev.off()
plot(p)
