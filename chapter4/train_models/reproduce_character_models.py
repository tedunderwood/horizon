...
def subset_balanced_by_authgender(modelname, metadatapath, numexamples, startdate, enddate):
    '''
    Creates metadata for a subset of characters that are equally balanced
    *both* by author gender and by character gender. This is used as a blank
    canvas when we're comparing different perspectives on gender.

    Note that numexamples here is 1/4 of the total dataset size.
    '''

    allmeta = pd.read_csv(metadatapath)
    timeslice = allmeta[(allmeta.firstpub >= startdate) & (allmeta.firstpub < enddate)]
    mm = timeslice[(timeslice.authgender == 'm') & (timeslice.gender == 'm')]
    mf = timeslice[(timeslice.authgender == 'm') & (timeslice.gender == 'f')]
    ff = timeslice[(timeslice.authgender == 'f') & (timeslice.gender == 'f')]
    fm = timeslice[(timeslice.authgender == 'f') & (timeslice.gender == 'm')]
    mmsample = mm.sample(n = numexamples)
    mfsample = mf.sample(n = numexamples)
    ffsample = ff.sample(n = numexamples)
    fmsample = fm.sample(n = numexamples)
    general_sample = pd.concat([mmsample, mfsample, ffsample, fmsample])
    outpath = '../models/' + modelname + '_meta.csv'
    general_sample.to_csv(outpath)
...
if __name__ == '__main__':
    ...
    if command == 'optimize_fifty_years':
        ...
        men = ['onlymalewritersA', 'onlymalewritersB', 'onlymalewritersC']
        women = ['onlywomenwritersA', 'onlywomenwritersB', 'onlywomenwritersC']
        test_subset_path = subset_balanced_by_authgender('balanced_test_metadata', metapath, numexamples = 500, startdate = 1800, enddate = 2000)
        generaloutpath = '/Users/tunder/Dropbox/python/character/perspectival_comparison/predictions/'
