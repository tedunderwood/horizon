...
hathi <- read.csv('../dataforR/authorratios.csv')
...
p <- ggplot() + 
  ...
  ggtitle('Percentage of fiction by women in HathiTrust (line)\nand Publishers Weekly (error bars)') +
  xlim(1850, 2007) + 
  theme_bw() +
...
tiff("../images/C4Fig10pubweekly.tiff", height = 6, width = 9, units = 'in', res=400)
plot(p)
dev.off()
plot(p)